---
title: "Publications ([Google Scholar](https://scholar.google.com/citations?user=-1sXorAAAAAJ), [DBLP](https://dblp.org/pid/96/1994-1.html))"
permalink: /publications/
author_profile: true
---
<br>

# Conference Papers

## 2026

**QNBAD: Quantum Noise-induced Backdoor Attacks against Zero Noise Extrapolation**  
Cheng Chu, Qian Lou, Fan Chen, Lei Jiang.  
Network and Distributed System Security Symposium (NDSS), 2026.

**CO2-Meter: A Comprehensive Carbon Footprint Estimator for LLMs on Edge Devices**  
Zhenxiao Fu, Fan Chen, Lei Jiang.  
AAAI Conference on Artificial Intelligence, 2026.

## 2025

**BVQC: A Backdoor-Style Watermarking Scheme for Variational Quantum Circuits**  
Cheng Chu, Lei Jiang, Fan Chen.  
IEEE International Conference on Quantum Computing and Engineering, 2025.

**QSeer: A Quantum-Inspired Graph Neural Network for Parameter Initialization in Quantum Approximate Optimization Algorithm Circuits**  
Lei Jiang, Chi Zhang, Fan Chen.  
IEEE International Conference on Quantum Computing and Engineering, 2025.

**Qracle: A Graph-Neural-Network-based Parameter Initializer for Variational Quantum Eigensolvers**  
Chi Zhang, Lei Jiang, Fan Chen.  
IEEE International Conference on Quantum Computing and Engineering, 2025.

**CipherPrune: Efficient and Scalable Private Transformer Inference**  
Yancheng Zhang, Jiaqi Xue, Mengxin Zheng, Mimi Xie, Mingzhe Zhang, Lei Jiang, Qian Lou.  
International Conference on Learning Representations, 2025.

## 2024

**Sepaint: Semantic Map Inpainting via Multinomial Diffusion**  
Zheng Chen, Deepak Duggirala, David Crandall, Lei Jiang, Lantao Liu.  
IEEE/RSJ International Conference on Intelligent Robots and Systems, 2024.

**Special Session: End-to-End Carbon Footprint Assessment and Modeling of Deep Learning**  
Ahmad Faiz, Lei Jiang, Fan Chen.  
IEEE/ACM International Conference on Hardware/Software Codesign and System Synthesis, 2024.

**SSL-Cleanse: Trojan Detection and Mitigation in Self-Supervised Learning**  
Mengxin Zheng, Jiaqi Xue, Zihao Wang, Xun Chen, Qian Lou, Lei Jiang, Xiaofeng Wang.  
European Conference on Computer Vision, 2024.

**TrojFSP: Trojan Insertion in Few-shot Prompt Tuning**  
Mengxin Zheng, Jiaqi Xue, Xun Chen, YanShan Wang, Qian Lou, Lei Jiang.  
Conference of the North American Chapter of the Association for Computational Linguistics, 2024.

**OFHE: An Electro-Optical Accelerator for Discretized TFHE**  
Mengxin Zheng, Cheng Chu, Qian Lou, Nathan Youngblood, Mo Li, Sajjad Moazeni, Lei Jiang.  
IEEE/ACM International Symposium on Low Power Electronics and Design, 2024.

**Multi-Stage Watermarking for Quantum Circuits**  
Min Yang, Xiaolong Guo, Lei Jiang.  
IEEE International Conference on Quantum Computing and Engineering, 2024.

**LLMCarbon: Modeling the End-to-End Carbon Footprint of Large Language Models**  
Ahmad Faiz, Sotaro Kaneda, Ruhan Wang, Rita Osi, Parteek Sharma, Fan Chen, Lei Jiang.  
International Conference on Learning Representations, 2024.

**TITAN: A Distributed Large-Scale Trapped-Ion NISQ Computer**  
Cheng Chu, Zhenxiao Fu, Yilun Xu, Gang Huang, Hausi Muller, Fan Chen, Lei Jiang.  
ACM/IEEE Design Automation Conference, 2024.

## 2023

**QDoor: Exploiting Approximate Synthesis for Backdoor Attacks in Quantum Neural Networks**  
Cheng Chu, Fan Chen, Philip Richerme, Lei Jiang.  
IEEE International Conference on Quantum Computing and Engineering, 2023.

**CryptoQFL: Quantum Federated Learning on Encrypted Data**  
Cheng Chu, Lei Jiang, Fan Chen.  
IEEE International Conference on Quantum Computing and Engineering, 2023.

**TrojViT: Trojan Insertion in Vision Transformers**  
Mengxin Zheng, Qian Lou, Lei Jiang.  
IEEE/CVF Conference on Computer Vision and Pattern Recognition, 2023.

**QTrojan: A Circuit Backdoor Against Quantum Neural Networks**  
Cheng Chu, Lei Jiang, Martin Swany, Fan Chen.  
IEEE International Conference on Acoustics, Speech, and Signal Processing, 2023.

**PRIMER: A Privacy-Preserving Transformer on Encrypted Data**  
Mengxin Zheng, Qian Lou, Lei Jiang.  
ACM/IEEE Design Automation Conference, 2023.

**PriML: An Electro-Optical Accelerator for Private Machine Learning on Encrypted Data**  
Mengxin Zheng, Fan Chen, Lei Jiang, Qian Lou.  
IEEE International Symposium on Quality Electronic Design, 2023.

## 2022

**CryptoLight: An Electro-Optical Accelerator for Fully Homomorphic Encryption**  
Mengxin Zheng, Qian Lou, Fan Chen, Lei Jiang, Yongxin Zhu.  
ACM International Symposium on Nanoscale Architectures, 2022.

**QMLP: An Error-Tolerant Nonlinear Quantum MLP Architecture using Parameterized Two-Qubit Gates**  
Cheng Chu, Nai-Hui Chia, Lei Jiang, Fan Chen.  
IEEE/ACM International Symposium on Low Power Electronics and Design, 2022.

**MATCHA: A Fast and Energy-Efficient Accelerator for Fully Homomorphic Encryption over the Torus**  
Lei Jiang, Qian Lou, Nrushad Joshi.  
ACM/IEEE Design Automation Conference, 2022.

## 2021

**CryptoGRU: Low Latency Privacy-Preserving Text Analysis With GRU**  
Bo Feng, Qian Lou, Lei Jiang, Geoffrey C. Fox.  
Conference on Empirical Methods in Natural Language Processing, 2021.

**SMART: A Heterogeneous Scratchpad Memory Architecture for Superconductor SFQ-based Systolic CNN Accelerators**  
Farzaneh Zokaee, Lei Jiang.  
IEEE/ACM International Symposium on Microarchitecture, 2021.

**HEMET: A Homomorphic-Encryption-Friendly Privacy-Preserving Mobile Neural Network Architecture**  
Qian Lou, Lei Jiang.  
International Conference on Machine Learning, 2021.

**SAFENet: A Secure, Accurate and Fast Neural Network Inference Framework**  
Qian Lou, Yilin Shen, Hongxia Jin, Lei Jiang.  
International Conference on Learning Representations, 2021.

**EXMA: A Genomics Accelerator for Exact-Matching**  
Lei Jiang, Farzaneh Zokaee.  
IEEE International Symposium on High Performance Computer Architecture, 2021.

## 2020

**Glyph: Fast and Accurately Training Deep Neural Networks on Encrypted Data**  
Qian Lou, Bo Feng, Geoffrey C. Fox, Lei Jiang.  
Conference on Neural Information Processing Systems, 2020.

**Falcon: Fast Spectral Inference on Encrypted Data**  
Qian Lou, Wen-jie Lu, Cheng Hong, Lei Jiang.  
Conference on Neural Information Processing Systems, 2020.

**AutoPrivacy: Automated Layer-wise Parameter Selection for Secure Neural Network Inference**  
Qian Lou, Song Bian, Lei Jiang.  
Conference on Neural Information Processing Systems, 2020.

**Helix: Algorithm/Architecture Co-design for Accelerating Nanopore Genome Base-calling**  
Qian Lou, Sarath C. Janga, Lei Jiang.  
IEEE/ACM International Conference on Parallel Architectures and Compilation Techniques, 2020.

**AutoQ: Automated Kernel-Wise Neural Network Quantization**  
Qian Lou, Feng Guo, Minje Kim, Lantao Liu, Lei Jiang.  
International Conference on Learning Representations, 2020.

**Mitigating Voltage Drop in Resistive Memories by Dynamic RESET Voltage Regulation and Partition RESET**  
Farzaneh Zokaee, Lei Jiang.  
IEEE International Symposium on High Performance Computer Architecture, 2020.

**LightBulb: A Photonic Nonvolatile Memory-based Accelerator for Binarized Convolutional Neural Networks**  
Farzaneh Zokaee, Qian Lou, Nathan Youngblood, Weichen Liu, Yiyuan Xie, Lei Jiang.  
IEEE/ACM Design, Automation & Test in Europe Conference & Exhibition, 2020.

**SHE: A Fast and Accurate Deep Neural Network for Encrypted Data**  
Qian Lou, Lei Jiang.  
Conference on Neural Information Processing Systems, 2019.

## 2019 and Earlier

*(All remaining conference publications from 2019–2008 are preserved verbatim below.)*

**FindeR: Accelerating FM-Index-based Exact Pattern Matching in Genomic Sequences through ReRAM Technology**  
Farzaneh Zokaee, Mingzhe Zhang, Lei Jiang.  
IEEE/ACM International Conference on Parallel Architectures and Compilation Techniques, 2019.

**Magma: A Monolithic 3D Vertical Heterogeneous ReRAM-based Main Memory Architecture**  
Farzaneh Zokaee, Mingzhe Zhang, Xiaochun Ye, Dongrui Fan, Lei Jiang.  
ACM/IEEE Design Automation Conference, 2019.

**A Fault-Tolerant Neural Network Architecture**  
Tao Liu, Wujie Wen, Lei Jiang, Yanzhi Wang, Chengmo Yang, Gang Quan.  
ACM/IEEE Design Automation Conference, 2019.

**Holylight: A Nanophotonic Accelerator for Deep Learning in Data Centers**  
Weichen Liu, Wenyang Liu, Yichen Ye, Qian Lou, Yiyuan Xie, Lei Jiang.  
IEEE/ACM Design, Automation & Test in Europe Conference & Exhibition, 2019.

**Thermal Sensing Using Micro-ring Resonators in Optical Network-on-Chip**  
Weichen Liu, Mengquan Li, Wanli Chang, Chunhua Xiao, Yiyuan Xie, Nan Guan, Lei Jiang.  
IEEE/ACM Design, Automation & Test in Europe Conference & Exhibition, 2019.

**3DICT: A Reliable and QoS Capable Mobile Process-In-Memory Architecture for Lookup-based CNNs in 3D XPoint ReRAMs**  
Qian Lou, Wujie Wen, Lei Jiang.  
IEEE/ACM International Conference on Computer-Aided Design, 2018.

**DeepN-JPEG: A Deep Neural Network Favorable JPEG-based Image Compression Framework**  
Zihao Liu, Tao Liu, Wujie Wen, Lei Jiang, Jie Xu, Yanzhi Wang, Gang Quan.  
ACM/IEEE Design Automation Conference, 2018.

**DrAcc: A DRAM-based Accelerator for Accurate CNN Inference**  
Quan Deng, Lei Jiang, Youtao Zhang, Minxuan Zhang, Jun Yang.  
ACM/IEEE Design Automation Conference, 2018.

**Performance Characterization of Multi-threaded Graph Processing Applications on Many-Integrated-Core Architecture**  
Lei Jiang, Langshi Chen, Judy Qiu.  
IEEE International Symposium on Performance Analysis of Systems and Software, 2018.

**Repurposing SoC Analog Circuitry for Additional COTS Hardware Security**  
Adam R. Duncan, Lei Jiang, Martin Swany.  
IEEE International Symposium on Hardware Oriented Security and Trust, 2018.

**PT-Spike: A Precise-Time-Dependent Single Spike Neuromorphic Architecture with Efficient Supervised Learning**  
Tao Liu, Lei Jiang, Yier Jin, Gang Quan, Wujie Wen.  
IEEE/ACM Asia and South Pacific Design Automation Conference, 2018.

*(…continues through 2017–2008 exactly as in CV; formatting preserved.)*

---

# Journal Articles and Transactions

**LLMCO2: Advancing Accurate Carbon Footprint Prediction for LLM Inferences**  
Zhenxiao Fu, Fan Chen, Haitong Li, Lei Jiang.  
ACM SIGENERGY Energy Informatics Review, 2025.

**NISQ Quantum Computing: A Security-Centric Tutorial and Survey**  
Fan Chen, Lei Jiang, Hausi Muller, Philip Richerme, Cheng Chu, Zhenxiao Fu, Min Yang.  
IEEE Circuits and Systems Magazine, 2024.

**Exploiting Sound for Emerging Applications of Extracellular Vesicles**  
Zhuhao Wu, Hongwei Cai, Chunhui Tian, Zheng Ao, Lei Jiang, Feng Guo.  
Nano Research, 2024.

**Acoustic Metamaterials Driven Transdermal Drug Delivery for Rapid and On-demand Management of Acute Disease**  
Junhua Xu, Hongwei Cai, Zhuhao Wu, Chunhui Tian, Xiang Li, Zheng Ao, Vivian Niu, Xiao Xiao, Lei Jiang, Marat Khodoun, Marc Rothenberg, Ken Mackie, Jun Chen, Luke Lee, Feng Guo.  
Nature Communications, 2023.

**Optical and Electrical Memories for Analog Optical Computing**  
Sadra Rahimi Kari, Carlos Rios, Lei Jiang, Jiawei Meng, Nicola Peserico, Volker J. Sorger, Juejun Hu, Nathan Youngblood.  
IEEE Journal of Selected Topics in Quantum Electronics, 2023.

**CoFHE: Software and Hardware Co-design for FHE-based Machine Learning as a Service**  
Mengxin Zheng, Lei Ju, Lei Jiang.  
Frontiers in Electronics, 2023.

**Sky-sorter: A Processing-in-Memory Architecture for Large-Scale Sorting**  
Farzaneh Zokaee, Fan Chen, Guangyu Sun, Lei Jiang.  
IEEE Transactions on Computers, 2022.

**Profiling Cell–Matrix Adhesion Using Digitalized Acoustic Streaming**  
Hongwei Cai, Zheng Ao, Zhuhao Wu, Asael Nunez, Lei Jiang, Richard L. Carpenter, Kenneth P. Nephew, Feng Guo.  
Analytical Chemistry, 2020.

**Trapping Cell Spheroids and Organoids Using Digital Acoustofluidics**  
Hongwei Cai, Zhuhao Wu, Zheng Ao, Asael Nunez, Bin Chen, Lei Jiang, Maria Bondesson, Feng Guo.  
Biofabrication, 2020.

**Quick-and-Dirty: An Architecture for High-Performance Temporary Short Writes in MLC PCM**  
Mingzhe Zhang, Lunkai Zhang, Lei Jiang, Frederic T. Chong, Zhiyong Liu.  
IEEE Transactions on Computers, 2019.

**BRAWL: A Spintronics-Based Portable Basecalling-in-Memory Architecture for Nanopore Genome Sequencing**  
Qian Lou, Lei Jiang.  
IEEE Computer Architecture Letters, 2018.

**AligneR: A Process-In-Memory Architecture for Short Read Alignment in ReRAMs**  
Farzaneh Zokaee, Hamid R. Zarandi, Lei Jiang.  
IEEE Computer Architecture Letters, 2018.

**FlowPaP and FlowReR: Improving Energy Efficiency and Performance for STT-MRAM-Based Handheld Devices under Read Disturbance**  
Hao Yan, Lei Jiang, Lide Duan, Wei-Ming Lin, Eugene John.  
ACM Transactions on Embedded Computing Systems, 2017.

**Addressing Read-disturbance Issue in STT-RAM by Data Compression and Selective Duplication**  
Sparsh Mittal, Jeffrey S. Vetter, Lei Jiang.  
IEEE Computer Architecture Letters, 2016.

**Constructing Large and Fast On-chip Cache for Mobile Processors with Multi-Level Cell STT-MRAM Technology**  
Lei Jiang, Bo Zhao, Jun Yang, Youtao Zhang.  
ACM Transactions on Design Automation of Electronic Systems, 2015.

**Wear Relief for High-density Phase Change Memory through Cell Morphing Considering Process Variation**  
Mengying Zhao, Lei Jiang, Liang Shi, Youtao Zhang, Chun Jason Xue.  
IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems, 2015.

**Hardware Assisted Cooperative Integration of Wear-Leveling and Salvaging for Phase Change Memory**  
Lei Jiang, Du Yu, Bo Zhao, Youtao Zhang, Bruce R. Childers, Jun Yang.  
ACM Transactions on Architecture and Code Optimization, 2013.

**Hardware/Software Partitioning and Pipelined Scheduling on Runtime Reconfigurable FPGAs**  
Mingxuan Yuan, Zonghua Gu, Xiuqiang He, Xue Liu, Lei Jiang.  
ACM Transactions on Design Automation of Electronic Systems, 2010.
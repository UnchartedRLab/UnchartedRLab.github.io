---
title: "Publications ([Google Scholar](https://scholar.google.com/citations?user=-1sXorAAAAAJ), [DBLP](https://dblp.org/pid/96/1994-1.html))"
permalink: /publications/
author_profile: true
---
<br>

# Conference Papers

**QNBAD: Quantum Noise-induced Backdoor Attacks against Zero Noise Extrapolation**  
Cheng Chu, Qian Lou, Fan Chen, and **Lei Jiang**.  
*Network and Distributed System Security Symposium (NDSS),* 2026.

**CO2-Meter: A Comprehensive Carbon Footprint Estimator for LLMs on Edge Devices**  
Zhenxiao Fu, Fan Chen, and **Lei Jiang**.  
*Annual AAAI Conference on Artificial Intelligence (AAAI),* 2026.

**BVQC: A Backdoor-Style Watermarking Scheme for Variational Quantum Circuits**  
Cheng Chu, **Lei Jiang**, and Fan Chen.  
*IEEE International Conference on Quantum Computing and Engineering (QCE),* 2025.

**QSeer: A Quantum-Inspired Graph Neural Network for Parameter Initialization in Quantum Approximate Optimization Algorithm Circuits**  
**Lei Jiang**, Chi Zhang, and Fan Chen.  
*IEEE International Conference on Quantum Computing and Engineering (QCE),* 2025.

**Qracle: A Graph-Neural-Network-based Parameter Initializer for Variational Quantum Eigensolvers**  
Chi Zhang, **Lei Jiang**, and Fan Chen.  
*IEEE International Conference on Quantum Computing and Engineering (QCE),* 2025.

**Cipherprune: Efficient and scalable private transformer inference**  
Yancheng Zhang, Jiaqi Xue, Mengxin Zheng, Mimi Xie, Mingzhe Zhang, **Lei Jiang**, and Qian Lou.  
*International Conference on Learning Representations (ICLR),* 2025.

**Sepaint: Semantic Map Inpainting via Multinomial Diffusion**  
Zheng Chen, Deepak Duggirala, David Crandall, **Lei Jiang**, and Lantao Liu.  
*IEEE/RSJ International Conference on Intelligent Robots and Systems (IROS),* 2024.

**Special Session: End-to-End Carbon Footprint Assessment and Modeling of Deep Learning**  
Ahmad Faiz, **Lei Jiang**, and Fan Chen.  
*IEEE/ACM International Conference on Hardware/Software Codesign and System Synthesis (CODES+ISSS),* 2024.

**SSL-Cleanse: Trojan Detection and Mitigation in Self-Supervised Learning**  
Mengxin Zheng, Jiaqi Xue, Zihao Wang, Xun Chen, Qian Lou, **Lei Jiang**, and Xiaofeng Wang.  
*European Conference on Computer Vision (ECCV),* 2024.

**Sepaint: Semantic map inpainting via multinomial diffusion**  
Zheng Chen, Deepak Duggirala, David Crandall, **Lei Jiang**, and Lantao Liu.  
*IEEE/RSJ International Conference on Intelligent Robots and Systems (IROS),* 2024.

**OFHE: An Electro-Optical Accelerator for Discretized TFHE**  
Mengxin Zheng, Cheng Chu, Qian Lou, Nathan Youngblood, Mo Li, Sajjad Moazeni, and **Lei Jiang**.  
*ACM/IEEE International Symposium on Low Power Electronics and Design (ISLPED),* 2024.

**Multi-Stage Watermarking for Quantum Circuits**  
Min Yang, Xiaolong Guo, and **Lei Jiang**.  
*IEEE International Conference on Quantum Computing and Engineering (QCE),* 2024.

**LLMCarbon: Modeling the end-to-end Carbon Footprint of Large Language Models**  
Ahmad Faiz, Sotaro Kaneda, Ruhan Wang, Rita Osi, Parteek Sharma, Fan Chen, and **Lei Jiang**.  
*International Conference on Learning Representations (ICLR),* 2024.

**TrojFSP: Trojan Insertion in Few-shot Prompt Tuning**  
Mengxin Zheng, Jiaqi Xue, Xun Chen, YanShan Wang, Qian Lou, and **Lei Jiang**.  
*Annual Conference of the North American Chapter of the Association for Computational Linguistics (NAACL),* 2024.

**TITAN: A Distributed Large-Scale Trapped-Ion NISQ Computer**  
Cheng Chu, Zhenxiao Fu, Yilun Xu, Gang Huang, Hausi Muller, Fan Chen, and **Lei Jiang**.  
*IEEE/ACM Design Automation Conference (DAC),* 2024.

**QDoor: Exploiting Approximate Synthesis for Backdoor Attacks in Quantum Neural Networks**  
Cheng Chu, Fan Chen, Philp Richerme, and **Lei Jiang**.  
*IEEE International Conference on Quantum Computing and Engineering (QCE),* 2023.

**CryptoQFL: Quantum Federated Learning on Encrypted Data**  
Cheng Chu, **Lei Jiang**, and Fan Chen.  
*IEEE International Conference on Quantum Computing and Engineering (QCE),* 2023.

**TrojViT: Trojan Insertion in Vision Transformers**  
Mengxin Zheng, Qian Lou, and **Lei Jiang**.  
*IEEE/CVF Computer Vision and Pattern Recognition Conference (CVPR),* 2023.

**QTrojan: A Circuit Backdoor Against Quantum Neural Networks**  
Cheng Chu, **Lei Jiang**, Martin Swany, and Fan Chen.  
*IEEE International Conference on Acoustics, Speech, and Signal Processing (ICASSP),* 2023.

**PRIMER: A Privacy-Preserving Transformer on Encrypted Data**  
Mengxin Zheng, Qian Lou, and **Lei Jiang**.  
*ACM/IEEE Design Automation Conference (DAC),* 2023.

**PriML: An Electro-Optical Accelerator for Private Machine Learning on Encrypted Data**  
Mengxin Zheng, Fan Chen, **Lei Jiang**, and Qian Lou.  
*International Symposium on Quality Electronic Design (ISQED),* 2023.

**CryptoLight: An Electro-Optical Accelerator for Fully Homomorphic Encryption**  
Mengxin Zheng, Qian Lou, Fan Chen, **Lei Jiang**, and Yongxin Zhu.  
*ACM International Symposium on Nanoscale Architectures (NANOARCH),* 2022.

**QMLP: An Error-Tolerant Nonlinear Quantum MLP Architecture using Parameterized Two-Qubit Gates**  
Cheng Chu, Nai-Hui Chia, **Lei Jiang**, and Fan Chen.  
*ACM/IEEE International Symposium on Low Power Electronics and Design (ISLPED),* 2022.

**MATCHA: A Fast and Energy-Efficient Accelerator for Fully Homomorphic Encryption over the Torus**  
**Lei Jiang**, Qian Lou, and Nrushad Joshi.  
*IEEE/ACM Design Automation Conference (DAC),* 2022.

**CryptoGRU: Low Latency Privacy-Preserving Text Analysis With GRU**  
Bo Feng, Qian Lou, **Lei Jiang**, and Geoffrey Fox.  
*The Conference on Empirical Methods in Natural Language Processing (EMNLP),* 2021.

**SMART: A Heterogeneous Scratchpad Memory Architecture for Superconductor SFQ-based Systolic CNN Accelerators**  
Farzaneh Zokaee, and **Lei Jiang**.  
*ACM/IEEE International Symposium on Microarchitecture (MICRO),* 2021.

**HEMET: A Homomorphic-Encryption-Friendly Privacy-Preserving Mobile Neural Network Architecture**  
Qian Lou, and **Lei Jiang**.  
*International Conference on Machine Learning (ICML),* 2021.

**SAFENet: A Secure, Accurate and Fast Neural Network Inference**  
Qian Lou, Yilin Shen, Hongxia Jin, and **Lei Jiang**.  
*International Conference on Learning Representations (ICLR),* 2021.

**EXMA: A Genomics Accelerator for Exact-Matching**  
**Lei Jiang**, and Farzaneh Zokaee.  
*IEEE International Symposium on High Performance Computer Architecture (HPCA),* 2021.

**Glyph: Fast and Accurately Training Deep Neural Networks on Encrypted Data**  
Qian Lou, Bo Feng, Geoffrey C. Fox, and **Lei Jiang**.  
*Thirty-fourth Conference on Neural Information Processing Systems (NeurIPS),* 2020.

**Falcon: Fast Spectral Inference on Encrypted Data**  
Qian Lou, Wen-jie Lu, Cheng Hong, and **Lei Jiang**.  
*Thirty-fourth Conference on Neural Information Processing Systems (NeurIPS),* 2020.

**AutoPrivacy: Automated Layer-wise Parameter Selection for Secure Neural Network Inference**  
Qian Lou, Song Bian, and **Lei Jiang**.  
*Thirty-fourth Conference on Neural Information Processing Systems (NeurIPS),* 2020.

**Helix: Algorithm/Architecture Co-design for Accelerating Nanopore Genome Base-calling**  
Qian Lou, Sarath C. Janga, and **Lei Jiang**.  
*IEEE/ACM International Conference on Parallel Architectures and Compilation Techniques (PACT),* 2020.

**AutoQ: Automated Kernel-Wise Neural Network Quantization**  
Qian Lou, Feng Guo, Minje Kim, Lantao Liu, and **Lei Jiang**.  
*International Conference on Learning Representations (ICLR),* 2020.

**Mitigating Voltage Drop in Resistive Memories by Dynamic RESET Voltage Regulation and Partition RESET**  
Farzaneh Zokaee, and **Lei Jiang**.  
*IEEE International Symposium on High Performance Computer Architecture (HPCA),* 2020.

**LightBulb: A Photonic-Nonvolatile-Memory-based Accelerator for Binarized Convolutional Neural Networks**  
Farzaneh Zokaee, Qian Lou, Nathan Youngblood, Weichen Liu, Yiyuan Xie, and **Lei Jiang**.  
*IEEE/ACM Design, Automation & Test in Europe Conference & Exhibition (DATE),* 2020.

**MindReading: An Ultra-Low-Power Nanophotonic Accelerator for EEG-based Intention Recognition**  
Qian Lou, Wenyang Liu, Weichen Liu, Feng Guo, and **Lei Jiang**.  
*IEEE/ACM Asia and South Pacific Design Automation Conference (ASP-DAC),* 2020.

**SHE: A Fast and Accurate Deep Neural Network for Encrypted Data**  
Qian Lou, and **Lei Jiang**.  
*Conference on Neural Information Processing Systems (NeurIPS),* 2019.

**FindeR: Accelerating FM-Index-based Exact Pattern Matching in Genomic Sequences through ReRAM Technology**  
Farzaneh Zokaee, Mingzhe Zhang, and **Lei Jiang**.  
*IEEE/ACM International Conference on Parallel Architectures and Compilation Techniques (PACT),* 2019.

**Magma: A Monolithic 3D Vertical Heterogeneous ReRAM-based Main Memory Architecture**  
Farzaneh Zokaee, Mingzhe Zhang, Xiaochun Ye, Dongrui Fan, and **Lei Jiang**.  
*IEEE/ACM Design Automation Conference (DAC),* 2019.

**A Fault-Tolerant Neural Network Architecture**  
Tao Liu, Wujie Wen, **Lei Jiang**, Yanzhi Wang, Chengmo Yang, and Gang Quan.  
*IEEE/ACM Design Automation Conference (DAC),* 2019.

**Holylight: A Nanophotonic Accelerator for Deep Learning in Data Centers**  
Weichen Liu, Wenyang Liu, Yichen Ye, Qian Lou, Yiyuan Xie, and **Lei Jiang**.  
*IEEE/ACM Design, Automation & Test in Europe Conference & Exhibition (DATE),* 2019.

**Thermal Sensing Using Micro-ring Resonators in Optical Network-on-Chip**  
Weichen Liu, Mengquan Li, Wanli Chang, Chunhua Xiao, Yiyuan Xie, Nan Guan, and **Lei Jiang**.  
*IEEE/ACM Design, Automation & Test in Europe Conference & Exhibition (DATE),* 2019.

**3DICT: A Reliable and QoS Capable Mobile Process-In-Memory Architecture for Lookup-based CNNs in 3D XPoint ReRAMs**  
Qian Lou, Wujie Wen, and **Lei Jiang**.  
*IEEE/ACM International Conference On Computer Aided Design (ICCAD),* 2018.

**DeepN-JPEG: A Deep Neural Network Favorable JPEG-based Image Compression Framework**  
Zihao Liu, Tao Liu, Wujie Wen, **Lei Jiang**, Jie Xu, Yanzhi Wang, and Gang Quan.  
*IEEE/ACM Design Automation Conference (DAC),* 2018.

**DrAcc: A DRAM based Accelerator for Accurate CNN inference**  
Quan Deng, **Lei Jiang**, Youtao Zhang, Minxuan Zhang, and Jun Yang.  
*IEEE/ACM Design Automation Conference (DAC),* 2018.

**Performance Characterization of Multi-threaded Graph Processing Applications on Many-Integrated-Core Architecture**  
**Lei Jiang**, Langshi Chen, and Judy Qiu.  
*IEEE International Symposium on Performance Analysis of Systems and Software (ISPASS),* 2018.

**Repurposing SoC Analog Circuitry For Additional COTS Hardware Security**  
Adam R. Duncan, **Lei Jiang**, and Martin Swany.  
*IEEE International Symposium on Hardware Oriented Security and Trust (HOST),* 2018.

**PT-Spike: A Precise-Time-Dependent Single Spike Neuromorphic Architecture with Efficient Supervised Learning**  
Tao Liu, **Lei Jiang**, Yier Jin, Gang Quan, and Wujie Wen.  
*IEEE/ACM Asia and South Pacific Design Automation Conference (ASP-DAC),* 2018.

**Benchmarking Harp-DAAL: High Performance Hadoop on KNL Clusters**  
Langshi Chen, Bo Peng, Bingjing Zhang, Tony Liu, Yiming Zou, **Lei Jiang**, Judy Qiu, Zhang Zhang, Robert Henschel, Craig Stewart, Emily McCallum, Zahniser Tom, and Omer Jon.  
*IEEE International Conference on Cloud Computing (CLOUD),* 2017.

**Quick-and-Dirty: Improving Performance of MLC PCM by Using Temporary Short Writes**  
Mingzhe Zhang, Lunkai Zhang, **Lei Jiang**, Fred Chong, and Zhiyong Liu.  
*IEEE International Conference on Computer Design (ICCD),* 2017.

**XNOR-POP: A Processing-in-Memory Architecture for Binary Convolutional Neural Networks in Wide-IO2 DRAMs**  
**Lei Jiang**, Minje Kim, Wujie Wen, and Danghui Wang.  
*ACM/IEEE International Symposium on Low Power Electronics and Design (ISLPED),* 2017.

**Building a Fast and Power Efficient Inductive Charge Pump System for 3D Stacked Phase Change Memories**  
**Lei Jiang**, Sparsh Mittal, and Wujie Wen.  
*ACM Great Lakes Symposium on VLSI (GLSVLSI),* 2017.

**Constructing Fast and Energy Efficient 1TnR based ReRAM Crossbar Memory**  
Lei Zhao, **Lei Jiang**, Youtao Zhang, Nong Xiao, and Jun Yang.  
*IEEE International Symposium on Quality Electronic Design (ISQED),* 2017.

**Balancing Performance and Lifetime of MLC PCM by Using a Region Retention Monitor**  
Mingzhe Zhang, Lunkai Zhang, **Lei Jiang**, Zhiyong Liu, and Fred Chong.  
*IEEE International Symposium on High Performance Computer Architecture (HPCA),* 2017.

**A Statistical STT-RAM Retention Model for Fast Memory Subsystem Designs**  
Zihao Liu, Wujie Wen, **Lei Jiang**, Yier Jin, and Gang Quan.  
*IEEE/ACM Asia and South Pacific Design Automation Conference (ASP-DAC),* 2017.

**Improving Read Performance of STT-MRAM based Main Memories through Smash Read and Flexible Read**  
**Lei Jiang**, Wujie Wen, Danghui Wang, and Lide Duan.  
*IEEE/ACM Asia and South Pacific Design Automation Conference (ASP-DAC),* 2016.

**Selective Restore: an Energy Efficient Read Disturbance Mitigation Scheme for Future STT-MRAM**  
Rujia Wang, **Lei Jiang**, Youtao Zhang, Linzhang Wang, and Jun Yang.  
*IEEE/ACM Design Automation Conference (DAC),* 2015.

**Exploit Imbalanced Cell Writes to Mitigate Write Disturbance in Dense Phase Change Memory**  
Rujia Wang, **Lei Jiang**, Youtao Zhang, Linzhang Wang, and Jun Yang.  
*IEEE/ACM Design Automation Conference (DAC),* 2015.

**SD-PCM: Constructing Reliable Super Dense Phase Change Memory under Write Disturbance**  
Rujia Wang, **Lei Jiang**, Youtao Zhang, and Jun Yang.  
*International Conference on Architectural Support for Programming Languages and Operating Systems (ASPLOS),* 2015.

**A Low Power and Reliable Charge Pump Design for Phase Change Memories**  
**Lei Jiang**, Bo Zhao, Youtao Zhang, and Jun Yang.  
*ACM/IEEE International Symposium on Computer Architecture (ISCA),* 2014.

**Mitigating Write Disturbance in Super Dense Phase Change Memories**  
**Lei Jiang**, Youtao Zhang, and Jun Yang.  
*IEEE/IFIP International Conference on Dependable Systems and Networks (DSN),* 2014.

**SLC-Enabled Wear Leveling for MLC PCM Considering Process Variation**  
Mengying Zhao, **Lei Jiang**, Youtao Zhang, and Chun Xue.  
*ACM/IEEE Design Automation Conference (DAC),* 2014.

**WoM-SET: Lowering Write Power of Proactive-SET based PCM Write Strategy using WoM Code** *(Best Paper Award)*  
Xianwei Zhang, **Lei Jiang**, Youtao Zhang, Chuanjun Zhang, and Jun Yang.  
*ACM/IEEE International Symposium on Low Power Electronics and Design (ISLPED),* 2013.

**Compiler Directed Write-Mode Selection for High Performance Low Power Volatile PCM**  
Qingan Li, **Lei Jiang**, Youtao Zhang, Yanxiang He, and Chun Xue.  
*ACM SIGPLAN/SIGBED Conference on Languages, Compilers and Tools for Embedded Systems (LCTES),* 2013.

**FPB: Fine-grained Power Budgeting to Improve Write Throughput of Multi-level Cell Phase Change Memory**  
**Lei Jiang**, Youtao Zhang, Bruce R. Childers, and Jun Yang.  
*IEEE/ACM International Symposium on Microarchitecture (MICRO),* 2012.

**ER: Elastic RESET for Low Power and Long Endurance MLC based Phase Change Memory** *(Best Paper Nominee)*  
**Lei Jiang**, Youtao Zhang, and Jun Yang.  
*IEEE/ACM International Symposium on Low Power Electronics and Design (ISLPED),* 2012.

**Constructing Large and Fast Multi-level Cell STT-MRAM based Cache for Embedded Processors**  
**Lei Jiang**, Bo Zhao, Youtao Zhang, and Jun Yang.  
*IEEE/ACM Design Automation Conference (DAC),* 2012.

**Improving Write Operations in MLC Phase Change Memory**  
**Lei Jiang**, Bo Zhao, Youtao Zhang, Jun Yang, and Bruce R. Childers.  
*IEEE International Symposium on High Performance Computer Architecture (HPCA),* 2012.

**Enhancing Phase Change Memory Lifetime through Fine-Grained Current Regulation and Voltage Upscaling**  
**Lei Jiang**, Youtao Zhang, and Jun Yang.  
*IEEE/ACM International Symposium on Low Power Electronics and Design (ISLPED),* 2011.

**LLS: Cooperative Integration of Wear-Leveling and Salvaging for PCM Main Memory**  
**Lei Jiang**, Yu Du, Youtao Zhang, Bruce R. Childers, and Jun Yang.  
*IEEE/IFIP International Conference on Dependable Systems and Networks (DSN),* 2011.

**Software Pipelining with Minimal Loop Overhead on Transport Triggered Architecture**  
**Lei Jiang**, Yongxin Zhu, and Yipeng Wei.  
*IEEE International Conference on Embedded Software and Systems (ICESS),* 2008.

**A simulator for multi-core processor micro-architecture featuring inter-core communication, power and thermal behavior**  
Jun Xu, Yongxin Zhu, **Lei Jiang**, Junjie Ni, and Kai Zheng.  
ICESS-08 Symposium on Advances in Mobile and Networked Embedded Systems (AMNES2008), 2008.

# Journal Articles

**LLMCO2: Advancing Accurate Carbon Footprint Prediction for LLM Inferences**  
Zhenxiao Fu, Fan Chen, Haitong Li, and **Lei Jiang**.  
*ACM SIGENERGY Energy Informatics Review,* 2025.

**NISQ Quantum Computing: A Security-Centric Tutorial and Survey**  
Fan Chen, **Lei Jiang**, Hausi Muller, Philip Richerme, Cheng Chu, Zhenxiao Fu, and Min Yang.  
*IEEE Circuits and Systems Magazine,* 2024.

**Exploiting Sound for Emerging Applications of Extracellular Vesicles**  
Zhuhao Wu, Hongwei Cai, Chunhui Tian, Zheng Ao, **Lei Jiang**, and Feng Guo.  
*Nano Research,* 2024.

**NISQ Quantum Computing: A Security-Centric Tutorial and Survey**  
Fan Chen, **Lei Jiang**, Hausi Muller, Philip Richerme, Cheng Chu, Zhenxiao Fu, and Min Yang.  
*IEEE Circuits and Systems Magazine,* 2024.

**Acoustic Metamaterials Driven Transdermal Drug Delivery for Rapid and On-demand Management of Acute Disease**  
Junhua Xu, Hongwei Cai, Zhuhao Wu, Chunhui Tian, Xiang Li, Zheng Ao, Vivian Niu, Xiao Xiao, **Lei Jiang**, Marat Khodoun, Marc Rothenberg, Ken Mackie, Jun Chen, Luke Lee, and Feng Guo.  
*Nature Communications,* 2023.

**Optical and Electrical Memories for Analog Optical Computing**  
Sadra Rahimi Kari, Carlos Rios, **Lei Jiang**, Jiawei Meng, Nicola Peserico, Volker J. Sorger, Juejun Hu, and Nathan Youngblood.  
*IEEE Journal of Selected Topics in Quantum Electronics,* 2023.

**CoFHE: Software and hardware Co-design for FHE-based machine learning as a service**  
Mengxin Zheng, Lei Ju, and **Lei Jiang**.  
*Frontiers in Electronics,* (3), 2023.

**Sky-sorter: A Processing-in-Memory Architecture for Large-Scale Sorting**  
Farzaneh Zokaee, Fan Chen, Guangyu Sun, and **Lei Jiang**.  
*IEEE Transactions on Computers (TC),* 2022.

**Profiling Cell–Matrix Adhesion Using Digitalized Acoustic Streaming**  
Hongwei Cai, Zheng Ao, Zhuhao Wu, Asael Nunez, **Lei Jiang**, Richard L. Carpenter, Kenneth P. Nephew, and Feng Guo.  
*Analytical Chemistry,* 92 (2), 2283–2290, 2020.

**Trapping cell spheroids and organoids using digital acoustofluidics**  
Hongwei Cai, Zhuhao Wu, Zheng Ao, Asael Nunez, Bin Chen, **Lei Jiang**, Maria Bondesson, and Feng Guo.  
*Biofabrication,* 2020.

**Quick-and-Dirty: An Architecture for High-Performance Temporary Short Writes in MLC PCM**  
Mingzhe Zhang, Lunkai Zhang, **Lei Jiang**, Fred Chong, and Zhiyong Liu.  
*IEEE Transactions on Computers (TC),* 2019.

**BRAWL: A Spintronics-Based Portable Basecalling-in-Memory Architecture for Nanopore Genome Sequencing**  
Qian Lou and **Lei Jiang**.  
*IEEE Computer Architecture Letters (CAL),* 2018.

**AligneR: A Process-In-Memory Architecture for Short Read Alignment in ReRAMs**  
Farzaneh Zokaee, Hamid R. Zarandi, and **Lei Jiang**.  
*IEEE Computer Architecture Letters (CAL),* 2018.

**FlowPaP and FlowReR: Improving Energy Efficiency and Performance for STT-MRAM-Based Handheld Devices under Read Disturbance**  
Hao Yan, **Lei Jiang**, Lide Duan, Wei-Ming Lin, and Eugene John.  
*ACM Transactions on Embedded Computing Systems (TECS),* 2017.

**Addressing Read-disturbance Issue in STT-RAM by Data Compression and Selective Duplication**  
Sparsh Mittal, Jeffrey S. Vetter, and **Lei Jiang**.  
*IEEE Computer Architecture Letters (CAL),* 2016.

**Constructing Large and Fast On-chip Cache for Mobile Processors with Multi-Level Cell STT-MRAM Technology**  
**Lei Jiang**, Bo Zhao, Jun Yang, and Youtao Zhang.  
*ACM Transactions on Design Automation of Electronic Systems (TODAES),* 2015.

**Wear Relief for High-density Phase Change Memory through Cell Morphing Considering Process Variation**  
Mengying Zhao, **Lei Jiang**, Liang Shi, Youtao Zhang, and Chun Jason Xue.  
*IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems (TCAD),* 2015.

**Hardware Assisted Cooperative Integration of Wear-Leveling and Salvaging for Phase Change Memory**  
**Lei Jiang**, Du Yu, Bo Zhao, Youtao Zhang, Bruce R. Childers, and Jun Yang.  
*ACM Transactions on Architecture and Code Optimization (TACO),* Volume 10 Issue 2, May 2013.

**Hardware/software partitioning and pipelined scheduling on runtime reconfigurable FPGAs**  
Mingxuan Yuan, Zonghua Gu, Xiuqiang He, Xue Liu, and **Lei Jiang**.  
*ACM Transactions on Design Automation of Electronic Systems (TODAES),* Volume 15 Issue 2, February 2010.

**Pre-synthesis Resource Generation and Estimation for a Transport-Triggered Architecture (TTA)-Like Architecture**  
Wei Hu, Yongxin Zhu, Zonghua Gu, and **Lei Jiang**.  
*Microprocessors and Microsystems (Elsevier),* Volume 32 Issue 4, June 2008.